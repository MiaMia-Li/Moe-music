...
export default {
  ...
  data () {
    return {
      scrollY: -1, // 观测实时滚动位置
      currentIndex: 0, // 当前显示第几个高亮
      diff: -1 // 区块滚动差
    }
  },
  created () { // 不需要观测，get/set
    this.probeType = 3
  ...
  methods: {
    ...
    onShortcutTouchMove (e) {
      ...
      let delta = (this.touch.y2 - this.touch.y1) / ANNCHOR_HEIGHT | 0
    ...
    scrollY (newY) {
      ...
      for (let i = 0; i < listHeight.length - 1; i++) {
        ...
        if (-newY >= height1 && -newY < height2) {
          ...
          this.diff = height2 + newY // 滚动时title的距离
          return
        }
      }
      // 滚动到底部
